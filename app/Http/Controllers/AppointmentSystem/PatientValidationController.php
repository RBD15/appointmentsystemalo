    ...
    public function validationUser(Request $request){
        ...
        if($request->has('document') && $request->has('phone_number')){
            ...
            if($result->phone_number==$request->phone_number){
                return response()->json(['message'=>'Patient Authorizated','name'=>$result->name,'age'=>$result->age,'address'=>$result->address,'phone_number'=>$result->phone_number,'plan'=>$result->plan_id,'status'=>$result->status],200);
            }
        }
